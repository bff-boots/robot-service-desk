...
const createTicket = asyncHandler(async (req,res)=>{
  const {product, description, serialNumber, faultType, priority} = req.body

  if(!product || !description || !serialNumber || !faultType){
    res.status(400)
    throw new Error('请补充设备型号、序列号、故障类型和故障描述')
  }
  ...
  const ticket = await Tickets.create({
    ...
    slaDueAt: new Date(Date.now() + (priority === 'P1' ? 2 : priority === 'P2' ? 8 : 24) * 60 * 60 * 1000),
    user: req.user.id,
    status: '新建'
  })
