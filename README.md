# Robot Service Desk｜机器人售后工单闭环系统

本项目基于 Support Desk MERN App（MIT）进行二次开发，面向机器人与智能硬件售后场景。

## 二次开发内容

- 将通用产品工单重构为巡检机器人、配送机器人、协作机械臂和视觉检测设备等场景。
- 增加设备序列号、故障类型、P1/P2/P3 优先级、处理工程师、SLA 截止时间和根因复盘字段。
- 工单状态调整为：新建、处理中、待客户确认、已关闭。
- 前端完成中文化，展示设备、故障、优先级与处理时限信息。

> 演示环境仅使用模拟工单，不对应真实客户、设备或维修结果。

原项目说明与 MIT 许可证保留在仓库中。
...
开发环境需要 MongoDB 才能使用登录、创建和更新真实工单。无需数据库也可以运行前端并访问 `/demo`，查看内置的模拟售后运营看板。

## 开源致谢

二次开发来源与许可证说明见 [NOTICE.md](NOTICE.md)。
